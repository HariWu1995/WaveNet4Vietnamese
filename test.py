...
tf.sg_arg_def(set=('train', "'train', 'valid', or 'test'.  The default is 'train'"))
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
	...
	saver.restore(sess, tf.train.latest_checkpoint('asset/train_vn'))
	
	# logging
	tf.sg_info('Testing started on %s set at global step[%08d].' % (tf.sg_arg().set.upper(), sess.run(tf.sg_global_step())))

	with tf.sg_queue_context():
		# create progress bar
		iterator = tqdm(range(0, int(data.num_batch * tf.sg_arg().frac)), total=int(data.num_batch * tf.sg_arg().frac), initial=0, desc='test', ncols=70, unit='b', leave=False)
		...
		for _ in iterator:
			...
			if batch_loss is not None 						\
						and not np.isnan(batch_loss.all()) 	\
						and not np.isinf(batch_loss.all()):
