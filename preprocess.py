...
def process_libri(csv_file, category):
			...
			for txt in txt_list:
				with open(txt, 'rt') as f:
					...
					for record in records:
                        ...
                        # wave file name
						wave_file = parent_path + '%s/%s/%s-%s-%s.flac' % (speaker, chapter, speaker, chapter, utterance)
	...
	for i, (wave_file, label) in enumerate(zip(wave_files, labels)):
		...
		# load flac file
		try:
			wave, sr, _ = audiolab.flacread(wave_file)
		except:
...
def convert_sph( sph, wav ):
	...
	try:
		subprocess.check_call(command) 	# not install sox yet? (apt-get install sox)
	except:
		pass

def process_ted(csv_file, category):
	parent_path = _data_path + 'TEDLIUM/TEDLIUM_release2/' + category + '/'
...
csv_f = open('asset/data/preprocess/meta/train.csv', 'a+')	# 'a+': append (write at end of file)
